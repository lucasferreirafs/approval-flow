...
import { useState, useRef, useEffect, useLayoutEffect, useId, KeyboardEvent } from "react"
    ...
    return (
        <div ref={wrapRef} className="relative select-none font-sans">
            ...
            {open && createPortal(
                <div
                    ref={dropdownRef}
                    id={dropdownId} // ID que corresponde ao aria-controls do trigger
                    role="listbox" // Role adequado para o container de opções
                    style={{
                        position: 'fixed',
                        top: `${position.top}px`,
                        ...
                        {filtered.length === 0 ? (
                            ...
                            filtered.map((opt) => {
                                ...
                                return (
                                    <div
                                        key={opt.value}
                                        role="option" // Role adequado para cada opção
                                        aria-selected={isSel} // Indica se está selecionado
